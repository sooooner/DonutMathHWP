...
def main():
    ...
    for hwp_path in tqdm(hwp_path_list):
        ...
        try:
            html_content_endnote, uuid_to_eqn_endnote = Hwp().extract_endnote_html(hwp_path, visible=False)
        except:
            continue

        endnotes, endnote_positions = HtmlPhaser(html_content_endnote).extract_endnote_info(uuid_to_eqn_endnote)
